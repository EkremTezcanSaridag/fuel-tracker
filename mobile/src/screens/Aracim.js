    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Pressable onPress={handleSave} style={({ pressed }) => [styles.saveButton, pressed && styles.pressed]}>
          ...
          <Text style={styles.saveButtonText}>Hesapla</Text>
        </Pressable>
        ...
        <View style={styles.historyTop}>
          <Text style={styles.expenseTitle}>Gider geçmişi</Text>
        ...
        <View style={styles.historyCard}>
          {expenseHistory.length === 0 ? (
            <View style={styles.emptyHistory}>
              ...
              <Text style={styles.emptyHistoryText}>İlk gider kaydın Hesapla dediğinde burada görünecek.</Text>
            </View>
          ) : (
            expenseHistory.slice(0, 4).map((record, index) => (
              <View key={record.id} style={[styles.historyRow, index === 0 && styles.historyRowFirst]}>
                ...
                <View style={styles.historyCopy}>
                  <Text style={styles.historyTitle}>{record.city} · {record.fuelTitle}</Text>
                  <Text style={styles.historyMeta}>{formatHistoryDate(record.createdAt)} · {formatNumber(Number(record.distanceKm) || 0)} km</Text>
                </View>
                <View style={styles.historyAmount}>
                  <Text style={styles.historyAmountValue}>{formatCurrency(Number(record.spentAmount) || 0)}</Text>
                  <Text style={styles.historyAmountMeta}>{formatCurrency(Number(record.costPerKm) || 0)} / km</Text>
                </View>
              </View>
            ))
          )}
...
const styles = StyleSheet.create({
  ...
  historyIcon: { alignItems: 'center', backgroundColor: colors.accentSoft, borderRadius: 8, height: 34, justifyContent: 'center', marginRight: 9, width: 34 },
  historyCopy: { flex: 1, paddingRight: 7 }, historyTitle: { color: colors.text, fontSize: 12, fontWeight: '800' }, historyMeta: { color: colors.muted, fontSize: 10, fontWeight: '700', marginTop: 3 },
  historyAmount: { alignItems: 'flex-end' }, historyAmountValue: { color: colors.text, fontSize: 12, fontWeight: '900' }, historyAmountMeta: { color: colors.mutedSoft, fontSize: 9, fontWeight: '700', marginTop: 3 },
